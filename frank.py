...
browser.get('http://www.pomona.edu/administration/dining/menus/frank.aspx')
...
def formatMeals(meal_list):
    ## Split on commas, and flatten into one list


    meal_list = map(splitComma, meal_list)
    meal_list = [m for sublist in meal_list for m in sublist]
    meal_list = map(strStrip, meal_list)
    return meal_list
